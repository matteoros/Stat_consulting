...
df <- df_meta |>
  ...
  select(Authors, ni, type, effect_size, V, std, lower_CI, upper_CI, SE, female_perc, white_ethnicity) 
```
...
```{r}
# Sample vector of author names
Authors <- c("Smith, John", "Doe, Jane", "Johnson, Robert")

# Extract the first names
first_names <- sapply(strsplit(Authors, ", "), function(x) x[1])

# Add "et al." after each first name
...
```{r}
library(robumeta)
mes_ss = robu(formula = yi ~ 1, data = df_r, studynum = Authoers,
